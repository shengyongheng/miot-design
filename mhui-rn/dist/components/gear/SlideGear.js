...
import { Animated, I18nManager, PanResponder, StyleSheet, Text, View } from 'react-native';
...
const DEFAULT_BLOCK_COLOR = '#FFFFFF'; // 滑块默认颜色
 ...
 * @property {string} leftTextOffColor - 组件置为disable时，左侧文字的颜色
 ...
 * @property {string} rightTextOffColor - 组件置为disable时，右侧文字的颜色
...
class SlideGear extends React.Component {
  ...
  static propTypes = {
    ...
    leftTextOffColor: PropTypes.string,
    //组件置为disable时，左侧文字的颜色
    leftTextStyle: Text.propTypes.style,
    maximumTrackTintColor: PropTypes.string,
    rightTextColor: PropTypes.string,
    rightTextOffColor: PropTypes.string,
    //组件置为disable时，右侧文字的颜色
  ...
  UNSAFE_componentWillMount() {
    ...
    this.state.pan.addListener(e => {
      ...
      if (I18nManager.isRTL) {
        backgroundWidth = -this.translateX + this.margin * 2 + this.blockWidth - this.state.dragToValueMin;
      } else {
        backgroundWidth = this.translateX + this.margin * 2 + this.blockWidth - this.state.dragToValueMin;
      }
  ...
  getClosetIndex(moveX) {
    let adjustCoord = 0; // 拖拽过程中Block的中心点坐标

    if (I18nManager.isRTL) {
      adjustCoord = this.endCoord - moveX + this.offset;
    } else {
      adjustCoord = moveX - this.offset;
    }
  ...
  renderDraggable() {
    ...
    const {
      colorScheme
    } = this.context; // 在没有找到自我定位的时候，要在舞台后面低调
    ...
    const whiteNoDark = new DynamicColor('#FFFFFF', 'rgba(255,255,255,1)');
    const greyNoDark = new DynamicColor('rgba(255,255,255,1)', 'rgba(255,255,255,1)');
  ...
  renderBackground() {
    const opacityValue = this.props.disabled ? 0.3 : 1; //左侧文字的颜色

    const leftTextColorValue = this.props.disabled && this.props.leftTextOffColor ? this.props.leftTextOffColor : this.props.leftTextColor;
    const {
      dragToValueMin: min,
      dragToValueMax: max
    ...
    let minimumTrackTintColor = this.props.minimumTrackTintColor ? this.props.minimumTrackTintColor : this.context.theme?.colorBtnGreenNor;

    if (this.props.disabled) {
      minimumTrackTintColor = this.context.theme?.colorSlideLeft;
    }
    ...
    return <Animated.View ref={background => this._background = background} style={{
      ...
      opacity: opacityValue
        ...
        height: this.blockHeight
            ...
            <Text accessible={false} focusable={false} style={[styles.text, {
  ...
  renderRightText() {
    const opacityValue = this.props.disabled ? 0.3 : 1; //右侧文字的颜色

    const rightTextColorValue = this.props.disabled && this.props.rightTextOffColor ? this.props.rightTextOffColor : this.props.rightTextColor;
    ...
    return <View style={[styles.textContainer, {
      ...
      opacity: opacityValue
        ...
        {this.props.showEndText ? <Text accessible={false} style={[styles.text, {
  ...
  render() {
    ...
    let maximumTrackTintColor = this.props.maximumTrackTintColor ? this.props.maximumTrackTintColor : this.context.theme?.colorBtnGrayNor;
    ...
    const leftTextColorValue = this.props.disabled && this.props.leftTextOffColor ? this.props.leftTextOffColor : this.props.leftTextColor; //右侧文字的颜色

    const rightTextColorValue = this.props.disabled && this.props.rightTextOffColor ? this.props.rightTextOffColor : this.props.rightTextColor;
    return <View onLayout={() => this._onLayout()} ref={container => this._container = container} style={[this.props.containerStyle, containerStyle]} {...getAccessibilityConfig({
