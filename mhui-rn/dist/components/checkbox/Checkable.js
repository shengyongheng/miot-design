...
import { Animated, ART, View, I18nManager } from 'react-native';
...
export default class Checkable extends React.Component {
  ...
  constructor(props, context) {
    ...
    const leftWidth = size * 0.43; // 左侧宽度

    const rightWidth = size - leftWidth; // 右侧宽度

    const intersectionHeight = Math.floor(size * 2 / 3); // 交点坐标

    const leftRectangleWidth = intersectionHeight / 1.5; // 左矩形的长度

    const rightRectangleWidth = size / 1.2; // 右矩形的长度
