...
export default class MultiButtons extends Component {
  ...
  getSelectors = () => {
    ...
    const itemCount = items.length;
    const sizeLevel = this.getSizeLevel(items);
    return items.map((item, index) => {
      const isHorizontal = itemCount === 2;
      const hasSeparator = isHorizontal;
      const hideTitle = itemCount >= 5 || items.findIndex(item => item && item.icon) === -1 || itemCount >= 3 && this.context.language !== 'zh';
      return <Fragment key={index}>
          {hasSeparator && index > 0 ? <View style={Styles.separator} /> : null}
          <CircleButton titleIsTouchable={this.props.titleIsTouchable} selected={this.props.activeIndex === index} showHighlight={this.props.activeIndex === -1} sizeLevel={sizeLevel} horizontal={isHorizontal} themeColor={themeColor} disabled={disabled || switchDisabled} title={hideTitle ? '' : item.title} icon={item.icon} iconSelected={item.iconSelected} iconText={item.title || ''} onPress={() => {
          this.onClick(index);
